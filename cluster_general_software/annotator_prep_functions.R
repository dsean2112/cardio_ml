...
#' @description
#' Simple function. Input is a vector, output is a vector
#' 
...
resample_wfdb_signal <- function(input, old_fs = NULL, new_fs = 500) {
  ...
  if (any(class(input) == 'egm')) {
    sig <- input$signal
    old_fs <- attributes(input$header)$record_line$frequency
  } else if (any(class(input) == 'signal_table')) {
    sig <- input
    if (is.null(old_fs)) {
      warning('Initial sample rate is not specified. Please specify, or input whole WFDB (with header)')
      break
    }
  }
  
  
  # Identify lead columns (everything except sample)
  ...
  resampled_leads <- lapply(lead_cols, function(col) {
    round(resample_ecg(sig[[col]], old_fs, new_fs))
  })
  names(resampled_leads) <- lead_cols
  
  # Convert to data.frame
  df <- as.data.frame(resampled_leads)
  
  # Add sample column
  df$sample <- seq_len(nrow(df)) - 1
  
  # Reorder columns: sample first
  df <- df[, c("sample", lead_cols)]
  
  # Pad or trim to exactly 5000 rows
  n <- nrow(df)
  if (n < target_n) {
    pad <- data.frame(
      sample = seq(n, target_n - 1),
      matrix(0, nrow = target_n - n, ncol = length(lead_cols))
    )
    names(pad) <- names(df)
    df <- rbind(df, pad)
  } else if (n > target_n) {
  ...
  # Return wfdb-style signal_table
...
predict_wfdb <- function(input,
  ...
  if (!is.null(output_name)) {
    ...
    if (nrow(output$annotation) > 0) { # ensure annotation table is not empty
      EGM::write_annotation(data = output$annotation,
                            annotator = output_ann_ext,
                            record = output_name,
                            record_dir = output_dir)
    } else {
      # check if ECG is blank (ie all 0s)
      error_name <- attributes(output$header)$record_line$record_name
      if (all(output$signal[1,-1] == 0)) {
        print(paste('ECG',error_name,'does not contain annotations. Blank ECG'))
      } else {
        print(paste('ECG',error_name,'does not contain annotations. Unknown reason'))
      }
    }
  }
...
# Predict (multi-sample, does not write data to disc) -----------------------------------------------------------------
...
predict_wfdb_multi <- function(input,
  ...
  # Re-order each annotation table
  ...
  if (input_class == 'wfdb') {
    for (i in seq_along(output)) {
      ...
      if (nrow(ann) > 0) { # ie if ECG has a flat signal, annotation set will be empty. If so, skip this step
        ...
        # check if ECG is blank (ie all 0s)
        error_name <- attributes(output[[i]]$header)$record_line$record_name
        if (all(output[[i]]$signal[1,-1] == 0)) {
          print(paste('ECG',error_name,'does not contain annotations. Blank ECG'))
        } else {
          print(paste('ECG',error_name,'does not contain annotations. Unknown reason'))
        }
